...
from myutils.data_augmentation import ADE20KSegmentation, get_mmseg_transforms_new
...
def evaluate_ade20k(model, dataloader, device, num_classes=150):
    """
    Evaluates a semantic segmentation model on the ADE20K validation set.
...
if LOAD_MODEL:
    model.load_state_dict(torch.load("/media/mlcv/SSD/Globalvit/segmentation_models/model_ade20k.pth"))
...
# --- Training Loop ---
    ...
    results = evaluate_ade20k(model, val_loader, device=DEVICE)
   # results2 = validate_coco_segmentation(model, val_dataset, save_dir="coco_val_results", device=DEVICE)
    print(results)
   # print(results2)
