...
from myutils.git_models import ViTFrontalizationEncoderDecoderLast, Discriminator, FaRLFrontalizationEncoderDecoder # Assume you save the model in this file
...
if LOAD_MODEL:
    model.load_state_dict(torch.load("/media/mlcv/SSD/outputs_pretrained_encoder_v2/model_epoch_02.pth"))
    Disc.load_state_dict(torch.load("/media/mlcv/SSD/outputs_pretrained_encoder_v2/disc_model_epoch_02.pth"))
...
# 2. Separate parameters into groups
# We filter by name to ensure we catch the right modules
optimizer = optim.AdamW([
    ...
    },
    {
        "params": [model.pos_embed] +
                  ...
                  list(model.decoder_head.parameters()), 
        "lr": LR,
    }
], lr=LR) # The 'lr' here acts as a default for any missed params
...
for group in optimizer.param_groups:
    for p in group['params']:
        grouped_params.add(p)

print("Missing params:", len(all_params - grouped_params))

# 3. Optional: Add a Scheduler
# This helps the decoder settle in once it has learned the basic structure
    ...
    for batch_idx, (inputs, targets, label_inter, label_intra) in enumerate(train_loader):
      ...
      #  targets = F.interpolate(targets, size=(2*IMAGE_HEIGHT, 2*IMAGE_WIDTH), mode='bilinear', align_corners=False)
       ...
       # refined_outputs = refine_module(outputs)
        ...
        identity_loss_value = identity_loss(arcface_outputs, arcface_targets, arcface_model, DEVICE) #+ identity_loss(arcface_outputs, arcface_inputs, arcface_model, DEVICE)
...
def main(args):
# --- Training Loop ---
    output_dir = Path("/media/mlcv/SSD/outputs_pretrained_encoder_v2") 
