...
from util.transformer import _get_clones, _get_activation_fn
...
class Dinov2SegmentationModelLast(nn.Module):
    def __init__(self, model_type='dinov2_vits14', img_size=770,
                 patch_size=14, nheads=8, num_decoder_layers=3,
                 num_classes=19, fpn_dim=None):
        super().__init__()
        ...
        fpn_dim = fpn_dim or self.embed_dim

        # 2. Multi-scale projections (P2–P5)
        self.proj = nn.ModuleList([
            nn.Conv2d(self.embed_dim, fpn_dim, 1) for _ in range(4)
        ])
        ...
        # 3. Token fusion after FPN
        self.feature_fusion = nn.Linear(fpn_dim * 4, self.embed_dim)

        # 4. Positional encoding (interpolated dynamically)
        self.pos_embed = nn.Parameter(torch.zeros(1, 1, self.embed_dim))
        nn.init.trunc_normal_(self.pos_embed, std=0.02)

        # 5. Your existing Transformer decoder
        decoder_layer = nn.TransformerDecoderLayer(
        ...
        # 6. Your existing head
        self.up_and_merge = nn.ConvTranspose2d(
            self.embed_dim, 256, kernel_size=3, stride=2, padding=1, output_padding=1
        )

        self.post_skip_conv = nn.Sequential(
            nn.Conv2d(256 + fpn_dim, 256, kernel_size=3, padding=1),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        # ---- Multi-scale DINOv2 features ----
        feats = self.backbone.get_intermediate_layers(x, n=4, reshape=True)
        # feats: shallow -> deep

        p = [self.proj[i](feats[i]) for i in range(4)]

        # FPN top-down fusion
        for i in range(3, 0, -1):
            p[i-1] = p[i-1] + F.interpolate(
                p[i], size=p[i-1].shape[-2:], mode="bilinear", align_corners=False
            )

        p = [self.fpn[i](p[i]) for i in range(4)]

        # ---- Build token memory from multi-scale pyramid ----
        # Resize all levels to same spatial size (highest resolution)
        target_hw = p[0].shape[-2:]
        p_resized = [
            p[0],
            F.interpolate(p[1], size=target_hw, mode="bilinear", align_corners=False),
            F.interpolate(p[2], size=target_hw, mode="bilinear", align_corners=False),
            F.interpolate(p[3], size=target_hw, mode="bilinear", align_corners=False),
        ]

        memory_spatial = torch.cat(p_resized, dim=1)  # [B, 4*fpn_dim, H', W']
        memory_seq = memory_spatial.flatten(2).transpose(1, 2)  # [B, N, 4*fpn_dim]
        memory = self.feature_fusion(memory_seq)  # [B, N, embed_dim]

        # ---- Your existing Transformer path ----
        pos = self.pos_embed.expand(1, memory.shape[1], -1)
        tgt = memory + pos
        decoded = self.decoder(tgt, memory)

        h_feat, w_feat = target_hw
        feature_map = decoded.transpose(1, 2).reshape(B, self.embed_dim, h_feat, w_feat)

        # Skip from shallow pyramid level
        out = self.up_and_merge(feature_map)
        skip_feature = F.interpolate(p[0], size=out.shape[-2:], mode='bilinear', align_corners=False)

        out = torch.cat([out, skip_feature], dim=1)
        out = self.post_skip_conv(out)

        logits = self.remaining_head(out)
