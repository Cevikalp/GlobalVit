...
cityscapes_weights = torch.tensor(
    [2.6, 6.0, 3.0, 10.0, 10.0, 10.0, 15.0, 15.0,
     4.0, 3.0, 3.0, 8.0, 8.0, 5.0, 10.0, 10.0,
     10.0, 10.0, 8.0],
    device=DEVICE
)
...
if LOAD_MODEL:
    model.load_state_dict(torch.load("/media/mlcv/Data/SegmentationProject/outputs_model_cityscapes_dino/model_epoch_best_full_transformer_last_sota.pth"))



#optimizer = optim.AdamW(model.parameters(), lr=LR)
optimizer = torch.optim.AdamW([
    {"params": model.backbone.parameters(), "lr": 0.5e-6},
    {
...
criterion = CityscapesLoss(class_weights=cityscapes_weights).to(DEVICE)
...
def main():
# --- Training Loop ---
    output_dir = Path("outputs_model_cityscapes_dino/")
    ...
    for epoch in range(1, EPOCHS + 1):
       ...
       # print(f"Epoch {epoch}: Train Loss = {train_loss:.4f}, Val Loss = {val_loss:.4f}")
        ...
        if val_iou > best_val_acc:
            ...
            print(f"New best model saved with mIoU: {best_val_acc:.4f}")
